...
const DatePicker = ({
  ...
  const daysOfTheWeek = () => {
    ...
    for (let day = dayShift; day < 7 + dayShift; day++) {
      const weekDayName = new Date(0, 0, day % 7).toLocaleDateString(lang || undefined, {
        weekday: 'short'
      });
      const trimmedName = weekDayName.slice(0, -1);
      daysOfTheWeekArray.push( /*#__PURE__*/React.createElement("span", {
        key: day,
        style: {
  ...
  const clickOnDay = day => {
    ...
    const newDate =
    // MDY for US format :
    format === 'MDY' ? `${month}/${day}/${chosenYear}`
    // YMD for ISO format :
    : format === 'YMD' ? `${chosenYear}/${month}/${day}` :
  ...
  const daysOfTheMonth = () => {
    ...
    for (let i = 1; i <= firstDayIndex; i++) {
      ...
      monthPage.push( /*#__PURE__*/React.createElement("button", {
        key: "previousMonth-" + i,
  ...
  }, new Date(chosenYear, currentMonth).toLocaleString(lang || undefined, {
    month: 'long'
  })), /*#__PURE__*/React.createElement("button", {
    onClick: toNextMonth
